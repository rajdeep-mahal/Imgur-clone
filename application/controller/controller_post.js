...
const PostController = {
    createPost: function(req, res, next) {
        uploader.single('uploadImage')(req, res, (err) => {
            if(err) {
                res.status(200).json({status: 'ERROR', message: 'Only images/gifs are allowed.', redirect: '/postImage'});
            } else {
                ...
                if(!title || !description) {
                    return res.status(200).json({status: 'ERROR', message: 'Please include a title and description.', redirect: '/postImage'});
                }

                // Making sure the user is authenticated.
                if(!fk_userid) {
                    return res.status(200).json({status: 'ERROR', message: 'User is not authenticated', redirect: '/login'});
                }

                PostModel.create(title, description, fileUploaded, thumbnailDestination, fk_userid)
                    .then((postID) => {
                        if(postID) {
                            res.status(200).json({status: 'OK', message: 'Post uploaded successfully.', redirect: `/image?id=${postID}`})
                        } else {
                            res.status(200).json({status: 'ERROR', message: 'Post upload failed, try again.', redirect: '/postImage'})
                        }
                    })
                    .catch((err) => {

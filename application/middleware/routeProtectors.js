...
routeProtectors.isValidImageLink = (req, res, next) => {
    if(req.query['id'] === undefined) {
        ...
        errorPrint('No image id provided and redirect to index.')
        res.redirect('/');
    } else {
...
routeProtectors.isValidProfileLink = (req, res, next) => {
    if(req.query['u'] === undefined) {
        ...
        successPrint('Check for a session, redirect to users profile');
        if(req.session.username) {
            res.redirect(`/profile?u=${req.session.username}`);
        } else {
            // No profile username provided, redirect to index.
            errorPrint('No profile username provided and redirect to index');
            res.redirect('/');
        }
    } else {
...
routeProtectors.isValidSearchLink = (req, res, next) => {
    if(req.query['query'] === undefined) {
        ...
        errorPrint('No query provided and redirect to index');
        res.redirect('/');
    } else {
